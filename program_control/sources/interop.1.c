...
int main(){
  ...
  // Async Memcp requires pinned memory
  ...
  #pragma omp target nowait depend(out: x[0:N]) map(from: x[0:N]) device(dev)
  myVectorSet(N,  1.0, x);
  ...
  #pragma omp interop init(targetsync: obj) device(dev) \
                 depend(in: x[0:N]) depend(inout: y[0:N]) //get obj 4 syncing
  ...
  if(obj != omp_interop_none && id == omp_ifr_cuda) {
    ...
    cudaStream_t s= (cudaStream_t)omp_get_interop_ptr(obj, omp_ipr_targetsync,
                                                      &rc);
    if(rc != omp_irc_success) {
       fprintf(stderr,"ERROR: Failed to get %s stream, rt error= %d.\n",
               rt_name, rc);
       if(rc == omp_irc_no_value) 
          fprintf(stderr, "Parameters valid, no meaningful value available."); 
       exit(1);
    }

    cublasSetStream( handle,s );
    cudaMemcpyAsync( d_x, x, N*sizeof(double), cudaMemcpyHostToDevice, s );
    cudaMemcpyAsync( d_y, y, N*sizeof(double), cudaMemcpyHostToDevice, s );
    cublasDaxpy(     handle, N, &scalar, &d_x[0], 1, &d_y[0], 1 ) ;
    cudaMemcpyAsync( y, d_y, N*sizeof(double), cudaMemcpyDeviceToHost, s );
